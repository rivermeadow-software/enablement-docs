...
The following components (VM based or OS based) are utilized in migrations to Microsoft Hyper-V:

| Component | Description | Location | Migration Method |
|:-----------:|:-------------:|:----------:|:----------:|
| **Source Server** | The physical server, virtual machine, or cloud instance that will be migrated to Microsoft Hyper-V. Direct ineraction with the source server occurs during OS based migrations. | Source Environment | OS Based |
...
| **RiverMeadow Migration Appliance** | The RiverMeadow migration appliance is a virtual appliance (virtual machine) that is deployed to Microsoft Hyper-V host. The appliance is responsible for local migration orchestration operations such as provisioning Hyper-V virtual machines in tandem with RiverMeadow platform. | Target Environment | OS and VM Based |
| **RiverMeadow Source Worker Appliance** | The RiverMeadow source worker appliance is a virtual appliance (virtual machine) that is deployed into the source VMware vSphere deployment. The source worker appliance is responsible for mounting VMware virtual machine snapshots and facilitating the data transfer to target workers running on Microsoft Hyper-V. The appliance is automatically deployed into the source VMware vSphere environment from the RiverMeadow migration appliance. | Source Environment | VM Based |
